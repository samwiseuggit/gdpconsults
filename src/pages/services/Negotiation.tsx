...
import { FileSignature, Handshake, Scale, Target, CheckCircle2, ArrowUpRight } from 'lucide-react';
...
const features = [
  { icon: Handshake, key: 'deal' },
  { icon: Scale, key: 'conflict' },
  { icon: Target, key: 'strategy' },
  { icon: FileSignature, key: 'contracts' },
];

const benefits = [
  'service.neg.benefit.1',
  'service.neg.benefit.2',
  'service.neg.benefit.3',
  'service.neg.benefit.4',
  'service.neg.benefit.5',
];
...
export default function Negotiation() {
  ...
  const seoData = {
    en: {
      title: 'Negotiation Services - GPD Consulting | Strategic Deal Making Africa',
      description: 'Expert negotiation services for complex deals, partnerships, and contracts. We help you achieve optimal outcomes in every negotiation across African markets.',
      keywords: 'negotiation services Africa, deal making, contract negotiation, strategic negotiation, business negotiation Africa, GPD Consulting',
    },
    fr: {
      title: 'Services de Negociation - GPD Consulting | Negociation Strategique Afrique',
      description: 'Services experts de negociation pour les transactions complexes, les partenariats et les contrats. Nous vous aidons a obtenir des resultats optimaux dans chaque negociation sur les marches africains.',
      keywords: 'services negociation Afrique, conclure des accords, negociation contrats, negociation strategique, negociation commerciale Afrique, GPD Consulting',
    },
  };
  ...
  const breadcrumbs = [
    ...
    { name: language === 'fr' ? 'Negociation' : 'Negotiation', url: 'https://gdpconsults.ca/services/negotiation' },
  ];

  const structuredData = {
    '@context': 'https://schema.org',
    '@type': 'Service',
    name: language === 'fr' ? 'Negociation' : 'Negotiation',
    description: currentSeo.description,
    url: 'https://gdpconsults.ca/services/negotiation',
    provider: {
      '@type': 'Organization',
      name: 'GPD Consulting',
  ...
  return (
    <>
      <SEO
        ...
        canonicalUrl="https://gdpconsults.ca/services/negotiation"
        ...
        <section className="relative py-32 lg:py-40 px-4 md:px-8 bg-gray-950 overflow-hidden">
          ...
          <div className="relative z-10 max-w-7xl mx-auto">
            ...
            <h1 className="font-heading text-5xl md:text-6xl lg:text-7xl font-bold text-white max-w-4xl leading-tight mb-6">
              <span className="gradient-text">{language === 'fr' ? 'Negociation' : 'Negotiation'}</span>
            </h1>
            <p className="text-gray-400 text-xl max-w-2xl">
              {language === 'fr' 
                ? 'Atteindre des resultats optimaux dans chaque transaction.'
                : 'Achieving optimal outcomes in every deal.'}
            </p>
          </div>
        </section>
        ...
        <section className="animate-section py-24 lg:py-32 px-4 md:px-8 bg-white">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <h2 className="font-heading text-4xl md:text-5xl font-bold text-gray-900 mb-8 animate-item">
              {language === 'fr' ? 'Negociation ' : 'Strategic '}
              <span className="gradient-text">{language === 'fr' ? 'Strategique' : 'Deal Making'}</span>
            </h2>
            <p className="text-gray-600 text-lg md:text-xl leading-relaxed animate-item">
              {language === 'fr'
                ? 'Notre equipe de negociation possede une expertise eprouvee dans la conduite de negociations complexes. Nous vous aidons a obtenir les meilleurs resultats possibles tout en preservant les relations commerciales.'
                : 'Our negotiation team has proven expertise in conducting complex negotiations. We help you achieve the best possible outcomes while preserving business relationships.'}
            </p>
          </div>
        </section>
              ...
              {features.map((feature, index) => (
                ...
                >
                  ...
                  <h3 className="font-heading text-xl font-bold text-gray-900 mb-3">
                    {t(`service.neg.${feature.key}.title`)}
                  </h3>
                  <p className="text-gray-500 text-sm leading-relaxed">
                    {t(`service.neg.${feature.key}.desc`)}
                  </p>
                </div>
              ))}
            </div>
        ...
        <section className="animate-section py-24 px-4 md:px-8 bg-gray-950 relative overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto text-center relative">
            <h2 className="font-heading text-4xl md:text-5xl font-bold text-white mb-6 animate-item">
              ...
              <span className="gradient-text">{language === 'fr' ? 'Negocier ?' : 'Negotiate?'}</span>
            </h2>
            <p className="text-gray-400 text-lg md:text-xl mb-10 max-w-2xl mx-auto animate-item">
              {language === 'fr'
                ? 'Contactez-nous pour discuter de vos besoins en negociation.'
                : 'Contact us to discuss your negotiation needs.'}
            </p>
            <Link 
